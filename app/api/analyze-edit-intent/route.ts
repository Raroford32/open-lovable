...
export async function POST(request: NextRequest) {
  try {
    const { prompt, manifest, model = 'openrouter/openai/gpt-5.2-codex' } = await request.json();
    ...
    const actualModel = model.startsWith('openrouter/')
      ? model.replace('openrouter/', '')
      : model;
    const aiModel = openrouter(actualModel);
